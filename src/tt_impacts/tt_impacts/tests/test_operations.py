...
class ScaleImpactsTests(helpers.BaseTests):
    ...
    async def check_scale(self, target_types, scale):
        ...
        old_scaled_targets = await db.sql('''SELECT target_type, target, amount FROM targets_impacts
                                             WHERE target_type IN %(target_types)s''',
                                          {'target_types': target_types})

        old_unscaled_targets = await db.sql('''SELECT target_type, target, amount FROM targets_impacts
                                               WHERE target_type NOT IN %(target_types)s''',
                                            {'target_types': target_types})

        old_scaled_actors = await db.sql('''SELECT target_type, target, actor_type, actor, amount FROM actors_impacts
                                            WHERE target_type IN %(target_types)s''',
                                         {'target_types': target_types})

        old_unscaled_actors = await db.sql('''SELECT target_type, target, actor_type, actor, amount FROM actors_impacts
                                              WHERE target_type NOT IN %(target_types)s''',
                                           {'target_types': target_types})
        ...
        new_unscaled_targets = await db.sql('''SELECT target_type, target, amount FROM targets_impacts
                                               WHERE target_type NOT IN %(target_types)s''',
                                            {'target_types': target_types})

        new_scaled_actors = await db.sql('''SELECT target_type, target, actor_type, actor, amount FROM actors_impacts
                                             WHERE target_type IN %(target_types)s''',
                                         {'target_types': target_types})

        new_unscaled_actors = await db.sql('''SELECT target_type, target, actor_type, actor, amount FROM actors_impacts
                                              WHERE target_type NOT IN %(target_types)s''',
                                           {'target_types': target_types})
        ...
        for row in old_scaled_targets:
            round = math.ceil if row['amount'] < 0 else math.floor
            row['amount'] = int(round(row['amount'] * scale))
    ...
    async def test_has_targets(self):
        ...
        await self.check_scale(target_types=[impacts[0].target.type, impacts[2].target.type], scale=0.5)
