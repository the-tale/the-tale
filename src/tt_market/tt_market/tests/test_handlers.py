...
class StatisticsTests(helpers.BaseTests):
    ...
    async def test_time_iterval(self):
        ...
        request = await self.client.post('/statistics', data=market_pb2.StatisticsRequest(time_from=time.mktime(time_from.timetuple())+time_from.microsecond/10**6,
                                                                                          time_till=time.mktime(time_till.timetuple())+time_till.microsecond/10**6).SerializeToString())
        data = await self.check_success(request, market_pb2.StatisticsResponse)
