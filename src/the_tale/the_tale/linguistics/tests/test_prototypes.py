...
class VerificatorTests(testcase.TestCase):
    ...
    def test_get_verificators__with_old(self):
        ...
        self.assertEqual(verificators[0], prototypes.Verificator(text='1', externals={'hero': ('привидение', ''), 'level': (1, '')}))
        self.assertEqual(verificators[1], prototypes.Verificator(text='5', externals={'hero': ('герой', ''), 'level': (2, '')}))
        self.assertEqual(verificators[2], prototypes.Verificator(text='', externals={'hero': ('героиня', ''), 'level': (5, '')}))
        self.assertEqual(verificators[3], prototypes.Verificator(text='', externals={'hero': ('рыцарь', 'мн'), 'level': (1, '')}))
