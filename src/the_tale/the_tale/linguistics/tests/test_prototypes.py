...
class WordPrototypeTests(utils_testcase.TestCase):
    ...
    def test_create(self):
        ...
        with mock.patch('the_tale.linguistics.workers.linguistics_manager.Worker.cmd_game_dictionary_changed') as cmd_game_dictionary_changed:
            with mock.patch('the_tale.linguistics.storage.dictionary.refresh') as game_refresh:
                with self.check_not_changed(lambda: storage.dictionary.version):
    ...
    def test_save__in_game(self):
        ...
        with mock.patch('the_tale.linguistics.storage.dictionary.refresh') as game_refresh:
            with self.check_changed(lambda: storage.dictionary.version):
                with self.check_not_changed(prototypes.WordPrototype._db_count):
                    prototype.save()
...
class TemplatePrototypeTests(utils_testcase.TestCase):
    ...
    def setUp(self):
        ...
        self.key_1, self.key_2 = random.sample(lexicon_keys.LEXICON_KEY.records, 2)
    ...
    def test_create(self):
        ...
        restriction_1, restriction_2 = storage.restrictions.all()[:2]
    ...
    def test_update(self):
        ...
        restriction_1, restriction_2, restriction_3 = storage.restrictions.all()[:3]
    ...
    def test_sync_restrictions(self):
        ...
        group_1, group_2, group_3 = random.sample(restrictions.GROUP.records, 3)
...
class VerificatorTests(utils_testcase.TestCase):

    def setUp(self):
        super(VerificatorTests, self).setUp()
        self.key = lexicon_keys.LEXICON_KEY.ACTION_FIRST_STEPS_INITIATION
    ...
    def test_get_verificators__with_old(self):
        ...
        self.assertEqual(verificators[0], prototypes.Verificator(text='1',
                                                                 externals={'date': ('18 сухого месяца 183 года', ''),
                                                                            'time': ('9:20', ''),
                                                                            'hero': ('привидение', ''),
                                                                            'hero.weapon': ('нож', ''),
    ...
    def test_get_verificators__one_substitutions_type(self):
        groups = lexicon_logic.get_verificators_groups(key=lexicon_keys.LEXICON_KEY.PVP_USE_ABILITY_BLOOD, old_groups={})
...
class ContributionTests(utils_testcase.TestCase):
