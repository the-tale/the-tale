...
class TestAccountClanRequests(BaseTestRequests):
    ...
    def setUp(self):
        ...
        self.account_clan_url = dext_urls.url('accounts:clans:account-clan', account=self.account.id)

    def test_wrong_account_id(self):
        self.check_html_ok(self.request_html(dext_urls.url('accounts:clans:account-clan', account=666)), texts=['clans.account_clan.account.not_found'], status_code=404)
        self.check_html_ok(self.request_html(dext_urls.url('accounts:clans:account-clan', account='bla-bla')), texts=['clans.account_clan.account.wrong_format'])

    def test_no_clan(self):
        self.check_html_ok(self.request_html(dext_urls.url('accounts:clans:account-clan', account=self.account.id)), texts=['clans.account_clan.no_clan'])

    # change tests order to fix sqlite segmentation fault
    def test_1_success(self):
        clan = self.create_clan(self.account, 0)
        self.check_redirect(self.account_clan_url, dext_urls.url('accounts:clans:show', clan.id))
...
class TestIndexRequests(BaseTestRequests):
    ...
    def test_no_clans(self):
        self.check_html_ok(self.request_html(dext_urls.url('accounts:clans:')),
                           texts=[('pgf-no-clans-message', 1)])

    @mock.patch('the_tale.accounts.clans.conf.settings.CLANS_ON_PAGE', 4)
    def test_clans_2_pages(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('accounts:clans:', page=2)),
                           texts=[('a-%d' % i, 1) for i in range(4, 6)] + [('pgf-no-clans-message', 0)])

        self.check_redirect(dext_urls.url('accounts:clans:', page=3), dext_urls.url('accounts:clans:', page=2, order_by=relations.ORDER_BY.NAME.value))
...
class TestShowRequests(BaseTestRequests):
    ...
    def setUp(self):
        ...
        self.show_url = dext_urls.url('accounts:clans:show', self.clan.id)
...
class TestEditRequests(BaseTestRequests):
    ...
    def setUp(self):
        ...
        self.clan = self.create_clan(self.account, 0)
        self.edit_url = dext_urls.url('accounts:clans:edit', self.clan.id)
    ...
    def test_ownership(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('accounts:clans:edit', clan.id)), texts=['clans.not_owner'])
...
class TestUpdateRequests(BaseTestRequests):
    ...
    def setUp(self):
        ...
        self.clan = self.create_clan(self.account, 0)
        self.update_url = dext_urls.url('accounts:clans:update', self.clan.id)
        self.request_login(self.account.email)
    ...
    def test_ownership(self):
        ...
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:update', clan.id)), 'clans.not_owner')
        self.check_clan_old_data()
...
class TestRemoveRequests(BaseTestRequests):
    ...
    def setUp(self):
        ...
        self.clan = self.create_clan(self.account, 0)
        self.remove_url = dext_urls.url('accounts:clans:remove', self.clan.id)
        self.request_login(self.account.email)
    ...
    def test_ownership(self):
        ...
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:remove', clan.id)), 'clans.not_owner')
        self.assertEqual(prototypes.ClanPrototype._db_count(), 2)

    def test_not_empty(self):
        self.clan.add_member(self.accounts_factory.create_account())

        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:remove', self.clan.id)), 'clans.remove.not_empty_clan')
        self.assertEqual(prototypes.ClanPrototype._db_count(), 1)
...
class MembershipForClanRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.for_clan_url = dext_urls.url('accounts:clans:membership:for-clan')
        self.clan = self.create_clan(self.account, 0)
        self.request_login(self.account.email)
    ...
    def test_has_invite_rights(self):
        prototypes.MembershipPrototype._model_class.objects.all().update(role=relations.MEMBER_ROLE.MEMBER)
...
class MembershipForAccountRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.for_account_url = dext_urls.url('accounts:clans:membership:for-account')
        self.request_login(self.account.email)
...
class MembershipInviteDialogRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.invite_url = dext_urls.url('accounts:clans:membership:invite', account=self.account_2.id)
        self.request_login(self.account.email)
    ...
    def test_wrong_account(self):
        self.check_html_ok(self.request_ajax_html(dext_urls.url('accounts:clans:membership:invite', account=666)), texts=['clans.membership.invite.account.not_found'], status_code=404)
        self.check_html_ok(self.request_ajax_html(dext_urls.url('accounts:clans:membership:invite', account='bla-bla')), texts=['clans.membership.invite.account.wrong_format'])
    ...
    def test_invite_exist__from_account(self):
        prototypes.MembershipRequestPrototype.create(initiator=self.account_2,
                                                     account=self.account_2,
                                                     clan=self.clan,
                                                     text='invite',
                                                     type=relations.MEMBERSHIP_REQUEST_TYPE.FROM_ACCOUNT)
...
class MembershipRequestDialogRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.clan = self.create_clan(self.account, 0)
        self.account_2 = self.accounts_factory.create_account()
        self.request_url = dext_urls.url('accounts:clans:membership:request', clan=self.clan.id)
    ...
    def test_wrong_clan(self):
        self.check_html_ok(self.request_ajax_html(dext_urls.url('accounts:clans:membership:request', clan=666)), texts=['clans.membership.request.clan.not_found'], status_code=404)
        self.check_html_ok(self.request_ajax_html(dext_urls.url('accounts:clans:membership:request', clan='bla-bla')), texts=['clans.membership.request.clan.wrong_format'])
...
class MembershipInviteRequestsTests(BaseMembershipRequestsTests):
    ...
    def test_login_required(self):
        ...
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 0)
    ...
    def test_wrong_account(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:invite', account=666), self.post_data()), 'clans.membership.invite.account.not_found')
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:invite', account='bla-bla'), self.post_data()), 'clans.membership.invite.account.wrong_format')
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 0)
    ...
    def test_invite_exist__from_clan(self):
        ...
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 1)
    ...
    def test_success(self):
        ...
        with self.check_new_message(self.account_2.id, [self.account.id]):
            ...
            self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 3)

            invite = prototypes.MembershipRequestPrototype._db_get_object(2)
...
class MembershipRequestRequestsTests(BaseMembershipRequestsTests):
    ...
    def test_wrong_clan(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:request', clan=666), self.post_data()), 'clans.membership.request.clan.not_found')
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:request', clan='bla-bla'), self.post_data()), 'clans.membership.request.clan.wrong_format')
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 0)
    ...
    def test_success(self):
        ...
        with self.check_new_message(self.account.id, [self.account_2.id]):
            ...
            self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 3)
            request = prototypes.MembershipRequestPrototype._db_get_object(2)
...
class MembershipAcceptRequestRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.clan = self.create_clan(self.account, 0)
        self.account_2 = self.accounts_factory.create_account()
        self.request = prototypes.MembershipRequestPrototype.create(initiator=self.account_2,
                                                                    account=self.account_2,
                                                                    clan=self.clan,
                                                                    text='request',
                                                                    type=relations.MEMBERSHIP_REQUEST_TYPE.FROM_ACCOUNT)

        self.accept_url = dext_urls.url('accounts:clans:membership:accept-request', request=self.request.id)
        self.request_login(self.account.email)
    ...
    def test_wrong_request_type(self):
        prototypes.MembershipRequestPrototype._model_class.objects.all().update(type=relations.MEMBERSHIP_REQUEST_TYPE.FROM_CLAN)
    ...
    def test_wrong_request_id(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:accept-request', request=666)), 'clan.membership.accept_request.request.not_found')
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:accept-request', request='bla-bla')), 'clan.membership.accept_request.request.wrong_format')
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 1)
        self.assertEqual(prototypes.MembershipPrototype._db_count(), 1)
    ...
    def test_success(self):
        with self.check_new_message(self.account_2.id, [self.account.id]):
            ...
            self.assertTrue(prototypes.MembershipPrototype._db_get_object(1).role.is_MEMBER)
...
class MembershipAcceptInviteRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.clan = self.create_clan(self.account, 0)
        self.account_2 = self.accounts_factory.create_account()
        self.request = prototypes.MembershipRequestPrototype.create(initiator=self.account,
                                                                    account=self.account_2,
                                                                    clan=self.clan,
                                                                    text='request',
                                                                    type=relations.MEMBERSHIP_REQUEST_TYPE.FROM_CLAN)

        self.accept_url = dext_urls.url('accounts:clans:membership:accept-invite', request=self.request.id)
        self.request_login(self.account_2.email)
    ...
    def test_wrong_request_type(self):
        prototypes.MembershipRequestPrototype._model_class.objects.all().update(type=relations.MEMBERSHIP_REQUEST_TYPE.FROM_ACCOUNT)
        self.check_ajax_error(self.post_ajax_json(self.accept_url), 'clans.membership.request_not_from_clan')
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 1)
        self.assertEqual(prototypes.MembershipPrototype._db_count(), 1)

    def test_wrong_request_id(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:accept-invite', request=666)), 'clan.membership.accept_invite.request.not_found')
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:accept-invite', request='bla-bla')), 'clan.membership.accept_invite.request.wrong_format')
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 1)
        self.assertEqual(prototypes.MembershipPrototype._db_count(), 1)
...
class MembershipRejectRequestRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.reject_url = dext_urls.url('accounts:clans:membership:reject-request', request=self.request.id)
        self.request_login(self.account.email)
    ...
    def test_wrong_request_id(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:reject-request', request=666)), 'clan.membership.reject_request.request.not_found')
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:reject-request', request='bla-bla')), 'clan.membership.reject_request.request.wrong_format')
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 1)
        self.assertEqual(prototypes.MembershipPrototype._db_count(), 1)
...
class MembershipRejectInviteRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.reject_url = dext_urls.url('accounts:clans:membership:reject-invite', request=self.request.id)
        self.request_login(self.account_2.email)
    ...
    def test_wrong_request_id(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:reject-invite', request=666)), 'clan.membership.reject_invite.request.not_found')
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:reject-invite', request='bla-bla')), 'clan.membership.reject_invite.request.wrong_format')
        self.assertEqual(prototypes.MembershipRequestPrototype._db_count(), 1)
        self.assertEqual(prototypes.MembershipPrototype._db_count(), 1)
...
class MembershipRemoveFromClanRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.remove_url = dext_urls.url('accounts:clans:membership:remove-from-clan', account=self.account_2.id)
        self.request_login(self.account.email)
    ...
    def test_wrong_account_id(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:remove-from-clan', account=666)), 'clan.membership.remove_from_clan.account.not_found')
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:remove-from-clan', account='bla-bla')), 'clan.membership.remove_from_clan.account.wrong_format')
        self.assertEqual(prototypes.MembershipPrototype._db_count(), 2)

    def test_no_remove_righs(self):
        self.request_logout()
        self.request_login(self.account_2.email)
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:remove-from-clan', account=self.account.id)),
    ...
    @mock.patch('the_tale.accounts.clans.relations.MEMBER_ROLE.MEMBER.priority', relations.MEMBER_ROLE.LEADER.priority)
    ...
    def test_success(self):
        with self.check_new_message(self.account_2.id, [self.account.id]):
            self.check_ajax_ok(self.post_ajax_json(self.remove_url))
            self.assertEqual(prototypes.MembershipPrototype._db_count(), 1)
            membership = prototypes.MembershipPrototype._db_get_object(0)
...
class MembershipLeaveClanRequestsTests(BaseMembershipRequestsTests):
    ...
    def setUp(self):
        ...
        self.leave_url = dext_urls.url('accounts:clans:membership:leave-clan')
        self.request_login(self.account_2.email)
    ...
    def test_leader(self):
        self.request_logout()
        self.request_login(self.account.email)
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:clans:membership:leave-clan')), 'clans.membership.leave_clan.leader')
        self.assertEqual(prototypes.MembershipPrototype._db_count(), 2)
