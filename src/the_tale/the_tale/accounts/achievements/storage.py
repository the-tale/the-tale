...
class AchievementsStorage(utils_storage.PrototypeStorage):
    ...
    @property
    def PROTOTYPE(self):
        return prototypes.AchievementPrototype
    ...
    def verify_achievements(self, account_id, type, old_value, new_value):
        ...
        for achievement in self.by_type(type, only_approved=True):
            if achievement.check(old_value, new_value):
                prototypes.AccountAchievementsPrototype.give_achievement(account_id=account_id, achievement=achievement)
    ...
    def get_groups_statistics(self, account_achievements):
        ...
        statistics = {group: (account_count[group], groups_count[group]) for group in relations.ACHIEVEMENT_GROUP.records}
...
achievements = AchievementsStorage()
