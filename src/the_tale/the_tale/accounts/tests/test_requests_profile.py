...
class ProfileRequestsTests(utils_testcase.TestCase, third_party_helpers.ThirdPartyTestsMixin):
    ...
    def confirm_email(self, task_number):
        ...
        response = self.request_html(dext_urls.url('accounts:profile:confirm-email') + '?uuid=' + uuid)
        self.check_response_redirect(response, dext_urls.url('accounts:profile:edited'))
    ...
    def check_redirect_to_email_confirmation(self, response):
        self.check_ajax_ok(response, data={'next_url': dext_urls.url('accounts:profile:confirm-email-request')})
    ...
    def test_profile_page_unlogined(self):
        self.check_redirect(dext_urls.url('accounts:profile:show'),
                            logic.login_page_url(dext_urls.url('accounts:profile:show')))
    ...
    def test_profile_page__fast_account(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('accounts:profile:show')), texts=texts)
    ...
    def test_profile_page_logined(self):
        self.request_login(self.account.email)
        response = self.client.get(dext_urls.url('accounts:profile:show'))
        self.assertEqual(response.status_code, 200)

    def test_refuse_third_party__profile_edited(self):
        self.request_third_party_token(account=self.account)
        self.check_html_ok(self.request_html(dext_urls.url('accounts:profile:edited')),
                           texts=['third_party.access_restricted'])

    def test_profile_edited(self):
        self.request_login(self.account.email)
        response = self.client.get(dext_urls.url('accounts:profile:edited'))
        self.assertEqual(response.status_code, 200)

    def test_profile_confirm_email_request(self):
        self.request_login(self.account.email)
        response = self.client.get(dext_urls.url('accounts:profile:confirm-email-request'))
        self.assertEqual(response.status_code, 200)

    def test_refuse_third_party__confirm_email_request(self):
        self.request_third_party_token(account=self.account)
        self.check_html_ok(self.request_html(dext_urls.url('accounts:profile:confirm-email-request')),
                           texts=['third_party.access_restricted'])

    def test_profile_update_password(self):
        self.request_login(self.account.email)

        response = self.post_ajax_json(dext_urls.url('accounts:profile:update'),
    ...
    def test_refuse_third_party__update(self):
        self.request_third_party_token(account=self.account)
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:profile:update'),
                                               {'email': self.account.email,
                                                'password': '222222',
                                                'nick': 'test_user'}),
    ...
    def test_profile_update_fast_errors(self):
        ...
        response = self.post_ajax_json(dext_urls.url('accounts:profile:update'), {'email': 'test_user@test.ru'})
        self.check_ajax_error(response, 'form_errors')

        response = self.post_ajax_json(dext_urls.url('accounts:profile:update'), {'password': '111111'})
        self.check_ajax_error(response, 'form_errors')

        response = self.post_ajax_json(dext_urls.url('accounts:profile:update'), {'nick': 'test_nick'})
        self.check_ajax_error(response, 'form_errors')

        response = self.post_ajax_json(dext_urls.url('accounts:profile:update'), {'email': 'test_user@test.ru', 'nick': 'test_nick'})
        self.check_ajax_error(response, 'accounts.profile.update.empty_fields')

        response = self.post_ajax_json(dext_urls.url('accounts:profile:update'), {'email': 'test_user@test.ru', 'password': '111111'})
        self.check_ajax_error(response, 'form_errors')

        response = self.post_ajax_json(dext_urls.url('accounts:profile:update'), {'password': '111111', 'nick': 'test_nick'})
        self.check_ajax_error(response, 'form_errors')
    ...
    def test_refuse_third_party__confirm_email(self):
        self.request_login(self.account.email)
        self.post_ajax_json(dext_urls.url('accounts:profile:update'), {'email': 'test_user@test.ru', 'nick': 'test_nick'})
        ...
        self.request_third_party_token(account=self.account)
        self.check_ajax_error(self.client.get(dext_urls.url('accounts:profile:confirm-email') + '?uuid=' + uuid),
                              'third_party.access_restricted')
    ...
    def test_confirm_email__no_uuid(self):
        self.check_html_ok(self.request_html(dext_urls.url('accounts:profile:confirm-email')),
                           texts=['accounts.profile.confirm_email.no_uid'])

    def test_confirm_email__wrong_account(self):
        self.request_login(self.account.email)
        self.post_ajax_json(dext_urls.url('accounts:profile:update'),
                            {'email': 'test_user@test.ru',
                             'nick': 'test_nick'})
        ...
        self.check_html_ok(self.request_html(dext_urls.url('accounts:profile:confirm-email', uuid=task.uuid)),
    ...
    def test_confirm_email__wrong_task(self):
        ...
        self.check_html_ok(self.client.get(dext_urls.url('accounts:profile:confirm-email', uuid='wronguuid')),
    ...
    def test_confirm_email__already_processed(self):
        ...
        self.check_html_ok(self.client.get(dext_urls.url('accounts:profile:confirm-email', uuid=task.uuid)),
    ...
    def test_update_last_news_reminder_time_unlogined(self):
        self.check_ajax_error(self.post_ajax_json(dext_urls.url('accounts:profile:update-last-news-reminder-time')),
                              'common.login_required')

    def test_update_last_news_reminder_time(self):
        self.request_login(self.account.email)

        self.check_ajax_ok(self.post_ajax_json(dext_urls.url('accounts:profile:update-last-news-reminder-time')))
