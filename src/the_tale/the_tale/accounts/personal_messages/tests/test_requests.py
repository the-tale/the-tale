...
class NewRequestsTests(BaseRequestsTests):
    ...
    def test_recipient_is_sender(self):
        self.check_html_ok(self.post_ajax_html(url('accounts:messages:new'), {'recipients': self.account_1.id}),
...
class CreateRequestsTests(BaseRequestsTests):
    ...
    def test_recipient_is_owner(self):
        self.check_ajax_error(self.post_ajax_json(url('accounts:messages:create'), {'text': 'test-message', 'recipients': self.account_1.id}),
                              'current_user')
