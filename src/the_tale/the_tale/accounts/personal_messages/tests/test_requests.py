...
class IndexRequestsTests(BaseRequestsTests):
    ...
    def test_unlogined_index(self):
        request_url = utils_urls.url('accounts:messages:')
    ...
    def test_fast_account(self):
        ...
        self.account_1.save()
        self.check_html_ok(self.request_html(utils_urls.url('accounts:messages:')), texts=['common.fast_account'])
    ...
    def test_index_no_messages(self):
        ...
        self.check_html_ok(self.request_html(utils_urls.url('accounts:messages:')), texts=[('pgf-no-messages', 1)])
    ...
    def test_index_big_page_number(self):
        self.request_login(self.account_1.email)
        self.check_redirect(utils_urls.url('accounts:messages:') + '?page=2', utils_urls.url('accounts:messages:') + '?page=1')
...
class SentRequestsTests(BaseRequestsTests):
    ...
    def test_fast_account(self):
        ...
        self.account_1.is_fast = True
        self.account_1.save()
        self.check_html_ok(self.request_html(utils_urls.url('accounts:messages:sent')), texts=['common.fast_account'])

    def test_unlogined_sent(self):
        request_url = utils_urls.url('accounts:messages:sent')
        self.check_redirect(request_url, accounts_logic.login_page_url(request_url))
    ...
    def test_sent_no_messages(self):
        self.request_login(self.account_4.email)
        self.check_html_ok(self.request_html(utils_urls.url('accounts:messages:sent')), texts=[('pgf-no-messages', 1)])
...
class ConversationRequestsTests(BaseRequestsTests):
    ...
    def test_fast_account(self):
        ...
        self.account_1.is_fast = True
        self.account_1.save()
        self.check_html_ok(self.request_html(utils_urls.url('accounts:messages:conversation', contact=self.account_2.id)), texts=['common.fast_account'])

    def test_unlogined_sent(self):
        request_url = utils_urls.url('accounts:messages:conversation', contact=self.account_2.id)
        self.check_redirect(request_url, accounts_logic.login_page_url(request_url))
    ...
    def test_sent(self):
        ...
        self.check_html_ok(self.request_html(utils_urls.url('accounts:messages:conversation', contact=self.account_2.id)), texts=texts)

    def test_sent_no_messages(self):
        self.request_login(self.account_4.email)
        self.check_html_ok(self.request_html(utils_urls.url('accounts:messages:conversation', contact=self.account_2.id)), texts=[('pgf-no-messages', 1)])
...
class NewRequestsTests(BaseRequestsTests):
    ...
    def test_unlogined(self):
        self.request_logout()
        request_url = utils_urls.url('accounts:messages:new')
        self.check_redirect(request_url, accounts_logic.login_page_url(request_url))
    ...
    def test_fast_account(self):
        ...
        self.account_1.is_fast = True
        self.account_1.save()
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': self.account_2.id}), texts=['common.fast_account'])

    @mock.patch('the_tale.accounts.prototypes.AccountPrototype.is_ban_forum', True)
    def test_banned_account(self):
        self.request_login(self.account_1.email)
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': self.account_2.id}), texts=['common.ban_forum'])

    def test_wrong_recipient_id(self):
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': 'aaa'}),
                           texts=[('pgf-error-form_errors', 1)])

    def test_recipient_not_found(self):
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': 666}),
                           texts=[('pgf-error-unexisted_account', 1)])

    def test_recipient_is_sender(self):
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': self.account_1.id}),
                           texts=[('pgf-error-current_user', 1)])

    def test_answer_wrong_message_id(self):
        logic.send_message(self.account_2.id, [self.account_1.id], 'message_2_1 1')
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new', answer_to='aaa'), {'recipients': self.account_2.id}),
                           texts=[('pgf-error-answer_to.wrong_format', 1)])

    def test_answer_to_not_found(self):
        logic.send_message(self.account_2.id, [self.account_1.id], 'message_2_1 1')
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new', answer_to=666), {'recipients': self.account_2.id}),
    ...
    def test_answer_to_no_permissions(self):
        ...
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new', answer_to=message_id), {'recipients': self.account_2.id}),
                           texts=[('pgf-error-answer_to.wrong_value', 1)])

    def test_success(self):
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': self.account_2.id}),
                           texts=[('pgf-new-message-form', 1)])

    def test_success_multiply_accoutns(self):
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': ('%d,%d' % (self.account_2.id, self.account_3.id))}),
                           texts=[('pgf-new-message-form', 1),
    ...
    def test_sent_to_system_user(self):
        ...
        self.check_html_ok(self.post_ajax_html(utils_urls.url('accounts:messages:new'), {'recipients': recipients}),
...
class CreateRequestsTests(BaseRequestsTests):
    ...
    def test_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message'}), 'common.login_required')
    ...
    def test_fast_account(self):
        ...
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message'}), 'common.fast_account')
    ...
    def test_banned_account(self):
        ...
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message'}), 'common.ban_forum')

        total, messages = tt_services.personal_messages.cmd_get_sent_messages(self.account_1.id)
        self.assertEqual(total, 0)

    def test_wrong_recipient_id(self):
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': 'aaa'}),
                              'form_errors')
    ...
    def test_recipient_not_found(self):
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': '666'}),
                              'unexisted_account')
    ...
    def test_recipient_is_owner(self):
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': self.account_1.id}),
                              'current_user')
    ...
    def test_form_errors(self):
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': '', 'recipients': self.account_2.id}),
                              'form_errors')
    ...
    def test_success(self):
        with self.check_delta(post_service_prototypes.MessagePrototype._db_count, 1):
            self.check_ajax_ok(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': self.account_2.id}))
    ...
    def test_success_multiply_accoutns(self):
        self.check_ajax_ok(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': ('%d,%d' % (self.account_2.id, self.account_3.id))}))
    ...
    def test_sent_to_system_user(self):
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': ('%d,%d' % (self.account_2.id, accounts_logic.get_system_user().id))}),
                              'system_user')
    ...
    def test_sent_to_fast_user(self):
        ...
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': ('%d,%d' % (self.account_2.id, self.account_3.id))}),
                              'fast_account')
...
class DeleteRequestsTests(BaseRequestsTests):
    ...
    def test_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:delete', self.message_id)), 'common.login_required')
    ...
    def test_fast_account(self):
        ...
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:delete', self.message_id)), 'common.fast_account')
    ...
    def test_delete_no_permissions(self):
        ...
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:delete', self.message_id)), 'message_id.wrong_value')
    ...
    def test_delete_from_sender(self):
        ...
        self.check_ajax_ok(self.post_ajax_json(utils_urls.url('accounts:messages:delete', self.message_id)))
...
class DeleteAllRequestsTests(BaseRequestsTests):
    ...
    def test_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:delete-all')), 'common.login_required')
    ...
    def test_fast_account(self):
        ...
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:delete-all')), 'common.fast_account')

    def test_delete(self):
        self.request_login(self.account_1.email)

        self.check_ajax_ok(self.post_ajax_json(utils_urls.url('accounts:messages:delete-all')))
...
class DeleteConversationRequestsTests(BaseRequestsTests):
    ...
    def test_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:delete-conversation', contact=self.account_2.id)), 'common.login_required')
    ...
    def test_fast_account(self):
        ...
        self.check_ajax_error(self.post_ajax_json(utils_urls.url('accounts:messages:delete-conversation', contact=self.account_2.id)), 'common.fast_account')

    def test_delete(self):
        self.request_login(self.account_1.email)

        self.check_ajax_ok(self.post_ajax_json(utils_urls.url('accounts:messages:delete-conversation', contact=self.account_2.id)))
...
class NewMessagesNumberTests(BaseRequestsTests):
    ...
    def test_success__has_messages(self):
        ...
        self.check_ajax_ok(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': ('%d,%d' % (self.account_1.id, self.account_3.id))}))
        self.check_ajax_ok(self.post_ajax_json(utils_urls.url('accounts:messages:create'), {'text': 'test-message', 'recipients': ('%d,%d' % (self.account_1.id, self.account_3.id))}))
