...
ABILITIES_CHOICES = sorted([(key, ability.NAME) for key, ability in heroes_abilities.ABILITIES.items()], key=lambda x: x[1])


BASE_INDEX_FILTERS = [utils_list_filter.reset_element(),
                      utils_list_filter.choice_element('тип:', attribute='type', choices=[(None, 'все')] + sorted(list(tt_beings_relations.TYPE.select('value', 'text')), key=lambda x: x[1])),
...
def argument_to_mob_type(value):
    return tt_beings_relations.TYPE(int(value))
