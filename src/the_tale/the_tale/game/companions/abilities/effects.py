...
def quest_money_reward(name, value, text, description, rarity_delta, border_left, border_right, work_when_dead=False):
...
def huckster(name, value, text, description, rarity_delta, work_when_dead=False):
...
class ABILITIES(DjangoEnum):
    ...
    records = (
        ('OBSTINATE', 0, 'строптивый', 'слаженность растёт очень медленно', CoherenceSpeed(0.6), RARITY_LOWER, False),
        ('STUBBORN', 1, 'упрямый', 'слаженность растёт медленнее обычного', CoherenceSpeed(0.8), RARITY_LOW, False),
        ('BONA_FIDE', 2, 'добросовестный', 'слаженность растёт быстрее обычного', CoherenceSpeed(1.2), RARITY_BIG, False),
        ('MANAGING', 3, 'исполнительный', 'слаженность растёт очень быстро', CoherenceSpeed(1.40), RARITY_BIGER, False),

        ('AGGRESSIVE', 4, 'агрессивный', 'увеличивает агрессивность героя',
         ChangeHabits(habit_type=game_relations.HABIT_TYPE.PEACEFULNESS, habit_sources=(heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_AGGRESSIVE, )), RARITY_NEUTRAL, False),
        ('PEACEFUL', 5, 'миролюбивый', 'увеличивает миролюбие героя',
         ChangeHabits(habit_type=game_relations.HABIT_TYPE.PEACEFULNESS, habit_sources=(heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_PEACEFULL,)), RARITY_NEUTRAL, False),
        ('RESERVED', 6, 'сдержанный', 'склоняет героя к балансу между агрессивностью и миролюбием',
         ChangeHabits(habit_type=game_relations.HABIT_TYPE.PEACEFULNESS, habit_sources=(heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_PEACEFULL_NEUTRAL_1,
                                                                                        heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_PEACEFULL_NEUTRAL_2)), RARITY_NEUTRAL, False),
        ('CANNY', 7, 'себе на уме', 'склоняет героя к балансу между честью и бесчестием',
         ChangeHabits(habit_type=game_relations.HABIT_TYPE.HONOR, habit_sources=(heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_HONOR_NEUTRAL_1,
                                                                                 heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_HONOR_NEUTRAL_2)), RARITY_NEUTRAL, False),
        ('HONEST', 8, 'честный', 'увеличивает честь героя',
         ChangeHabits(habit_type=game_relations.HABIT_TYPE.HONOR, habit_sources=(heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_HONORABLE,)), RARITY_NEUTRAL, False),
        ('SNEAKY', 9, 'подлый', 'уменьшает честь героя',
         ChangeHabits(habit_type=game_relations.HABIT_TYPE.HONOR, habit_sources=(heroes_relations.HABIT_CHANGE_SOURCE.COMPANION_DISHONORABLE,)), RARITY_NEUTRAL, False),
        ...
        ('PACK', 14, 'вьючный', '2 дополнительных места в рюкзаке', MaxBagSize(2), RARITY_BIG, False),
        ('FREIGHT', 15, 'грузовой', '4 дополнительных места в рюкзаке', MaxBagSize(4), RARITY_BIGER, False),
        ('DRAFT', 16, 'тягловой', '6 дополнительных мест в рюкзаке', MaxBagSize(6), RARITY_BIGEST, False),

        ('PARAPHERNALIA', 55, 'личные вещи', 'забирает 2 места в рюкзаке для своих вещей', MaxBagSize(-2), RARITY_LOW, False),
        ('SPARE_PARTS', 56, 'запчасти', 'забирает 4 места в рюкзаке для запчастей', MaxBagSize(-4), RARITY_LOWER, False),
        ...
        ('KNOWN', 17, 'известный', 'находит политически важную работу, задания героя оказывают большее влияние на мир (максимальный бонус к влиянию: 100%)',
         PoliticsPower(0.5, 1.0), RARITY_BIGER, False),
        ('CAD', 18, 'хам', 'хамит горожанам, герою не доверяют политически важную работу, поэтому он оказывает меньшее влияние на мир (минимальный штраф к влиянию: -50%)',
          PoliticsPower(-1.0, -0.5), RARITY_LOWER, False),

        ('FIT_OF_ENERGY', 19, 'прилив сил', 'даёт небольшой бонус к физическому урону героя', PhysicDamageBonus(1.05, 1.1), RARITY_BIGER, False),
        ('PEP', 20, 'бодрость духа', 'даёт небольшой бонус к магическому урону героя', MagicDamageBonus(1.05, 1.1), RARITY_BIGER, False),

        ('SLOW', 22, 'медлительный', 'постоянный штраф к скорости героя', Speed(0.7, 0.85), RARITY_LOWER, False),
        ('SEDATE', 68, 'степенный', 'постоянный небольшой штраф к скорости героя', Speed(0.8, 0.9), RARITY_LOW, False),
        ('SLED', 21, 'ездовой', 'постоянный небольшой бонус к скорости героя', Speed(1.05, 1.10), RARITY_BIG, False),
        ('RACER', 23, 'скакун', 'постоянный бонус к скорости героя', Speed(1.1, 1.15), RARITY_BIGER, False),
        ('FLEET_FOOTED', 69, 'быстроногий', 'постоянный большой бонус к скорости героя', Speed(1.1, 1.2), RARITY_BIGEST, False),

        ('FIGHTER', 24, 'боец', 'немного увеличивает инициативу героя, в бою может применить способность «%s»' % BattleAbilityHit.ABILITY.NAME, BattleAbilityHit(), RARITY_BIGER, False),
        ('RAM', 25, 'громила', 'немного увеличивает инициативу героя, в бою может применить способность «%s»' % BattleAbilityStrongHit.ABILITY.NAME, BattleAbilityStrongHit(), RARITY_BIGER, False),
        ('HOUSEBREAKER', 26, 'таран', 'немного увеличивает инициативу героя, в бою может применить способность «%s»' % BattleAbilityRunUpPush.ABILITY.NAME, BattleAbilityRunUpPush(), RARITY_BIGER, False),
        ('ARSONIST', 27, 'поджигатель', 'немного увеличивает инициативу героя, в бою может применить способность «%s»' % BattleAbilityFireball.ABILITY.NAME, BattleAbilityFireball(), RARITY_BIGER, False),
        ('POISONER', 28, 'отравитель', 'немного увеличивает инициативу героя, в бою может применить способность «%s»' % BattleAbilityPoisonCloud.ABILITY.NAME, BattleAbilityPoisonCloud(), RARITY_BIGER, False),
        ('FROST', 29, 'морозко', 'немного увеличивает инициативу героя, в бою может применить способность «%s»' % BattleAbilityFreezing.ABILITY.NAME, BattleAbilityFreezing(), RARITY_BIGER, False),

        ('UNGAINLY', 30, 'неуклюжий', 'большой штраф к инициативе героя', Initiative(0.8, 0.875), RARITY_LOWER, False),
        ('CLUMSY', 31, 'неповоротливый', 'малый штраф к инициативе героя', Initiative(0.89, 0.94), RARITY_LOW, False),
        ('CLEVER', 32, 'ловкий', 'малый бонус к инициативе героя', Initiative(1.01, 1.06), RARITY_BIGER, False),
        ('IMPETUOUS', 33, 'стремительный', 'большой бонус к инициативе героя', Initiative(1.05, 1.125), RARITY_BIGER, False),

        ('NOISY', 34, 'шумный', 'так сильно шумит, что привлекает внимание большего количества врагов', BattleProbability(0.2, 0.1), RARITY_LOWER, False),
        ('SHARP_EYE', 36, 'острый глаз', 'издали высматривает врагов, снижая вероятность встречи с ними', BattleProbability(-0.05, -0.1), RARITY_BIGER, False),

        ('DEATHY', 35, 'смертельно страшный', 'распугивает всех, кого встречает, вероятность встретить врага стремится к нулю', Deathy(-1), RARITY_BIGEST, False),

        ('TORTURER', 37, 'терзатель', 'растерзывает врагов в бою так сильно, что уменьшается шанс найти уцелевшую в бою добычу', LootProbability(0.6, 0.8), RARITY_LOWER, False),
        ('HUNTER', 38, 'охотник', 'помогает герою сражаться аккуратнее, благодаря чему увеличивает шанс найти уцелевшую в бою добычу', LootProbability(1.1, 1.2), RARITY_BIGER, False),

        ('NOT_LIFER', 39, 'тщедушный', 'при ранении может получить дополнительный урон', CompanionDamage(c.COMPANIONS_DAMAGE_PER_WOUND), RARITY_LOWER, False),
        ('PUNY', 40, 'не жилец', 'при ранении может получить большой дополнительный урон', CompanionDamage(2 * c.COMPANIONS_DAMAGE_PER_WOUND), RARITY_LOWEST, False),

        ('CAMOUFLAGE', 41, 'камуфляж', 'благодаря незаметности, реже получает урон в бою', CompanionDamageProbability(0.9, 0.8), RARITY_BIGER, False),
        ('FLYING', 42, 'летающий', 'перемещаясь не только вокруг но и над противником, значительно реже получает урон в бою', CompanionDamageProbability(0.85, 0.7), RARITY_BIGEST, False),

        ('PICKPOCKET', 43, 'карманник', 'В каждом городе крадёт из карманов горожан немного денег', CompanionStealMoney(0.5, 3.0), RARITY_BIG, False),
        ('ROBBER', 44, 'грабитель', 'В каждом городе крадёт у горожан что-нибудь полезное, возможно, даже экипировку', CompanionStealItem(1.05, 4.0), RARITY_BIGER, False),

        ('COSTLY', 45, 'дорогой', 'при потере спутника герой получает весьма дорогие запчасти, обращаемые в деньги.', CompanionSpareParts(), RARITY_BIG, False),

        ('WISE', 46, 'мудрый', 'спутник иногда делится мудростью с героем, давая тому немного опыта.', CompanionSayWisdom(0.5, 1.0), RARITY_BIGER, False),
        ('DIFFICULT', 47, 'сложный', 'ухаживая за спутником, герой может получить немного опыта', CompanionExpPerHeal(0.5, 1.0), RARITY_BIGER, False),

        ('FAN', 48, 'поклонник', 'возносит хвалу Хранителю вместе с героем и с небольшой вероятностью даёт бонусную энергию', DoubleEnergyRegeneration(0.05, 0.1), RARITY_BIGER, False),
        ('SAN', 49, 'сан', 'возносит хвалу Хранителю вместе с героем и с хорошей вероятностью даёт бонусную энергию', DoubleEnergyRegeneration(0.1, 0.2), RARITY_BIGEST, False),

        ('EAT_CORPSES', 50, 'пожиратель', 'после боя иногда ест труп врага, пополняя себе хиты. Не ест конструктов, нежить, демонов и стихийных существ.', CompanionEatCorpses(0.5, 1), RARITY_BIGER, False),
        ('REGENERATE', 51, 'регенерация', 'во время отдыха может восстановить своё здоровье', CompanionRegenerate(0.5, 1.0), RARITY_BIGER, False),

        ('EATER', 52, 'едок', 'при каждом посещении города герой тратит деньги на еду для спутника', CompanionEat(0.35, 0.25), RARITY_LOW, False),
        ('GLUTTONOUS', 53, 'прожорливый', 'при каждом посещении города герой тратит много денег на еду для спутника', CompanionEat(0.7, 0.5), RARITY_LOWER, False),
        ('INDEPENDENT', 54, 'самостоятельный', 'может кормиться сам, снижает стоимость кормёжки (способностей «едок» и «прожорливый»)', CompanionEatDiscount(0.9, 0.5), RARITY_BIG, False),

        ('DRINKER', 57, 'пьяница', 'спутник пропивает случайный предмет из рюкзака при посещении героем города', CompanionDrinkArtifact(0.95, 0.5), RARITY_BIG, False),

        ('EXORCIST', 58, 'экзорцист', 'спутник может изгнать встречного демона', CompanionExorcist(0.5, 1.0), RARITY_BIG, False),

        ('HEALER', 59, 'лекарь', 'ускоряет лечение героя на отдыхе', RestLenght(0.9, 0.5), RARITY_BIGEST, False),

        ('INSPIRATION', 60, 'воодушевление', 'воодушевляет героя на подвиги, снижая время бездействия между заданиями', IDLELenght(0.9, 0.5), RARITY_BIG, False),
        ('LAZY', 61, 'ленивый', 'ленится вместе с героем и увеличивает время бездействия между заданиями', IDLELenght(2.0, 1.5), RARITY_LOW, False),

        ('COWARDLY', 62, 'трусливый', 'реже защищает героя в бою', CompanionBlockProbability(0.5, 0.75), RARITY_NEUTRAL, False),
        ('BODYGUARD', 63, 'телохранитель', 'чаще защищает героя в бою', CompanionBlockProbability(1.1, 1.25), RARITY_NEUTRAL, False),
        ...
        ('CONTACT', 65, 'связной', 'служит маяком для Хранителя и увеличивает шанс критической помощи', EtherealMagnet(0.05, 0.1), RARITY_BIGEST, False),

        ('TELEPORTATOR', 66, 'телепортатор', 'периодически переносит героя между городами или ключевыми точками задания', CompanionTeleport(0.05, 0.1), RARITY_BIGEST, False),
        ('FLYER', 67, 'ездовой летун', 'часто  переносит героя на небольшое расстояние по воздуху', CompanionFly(0.05, 0.1), RARITY_BIGEST, False),

        ('UNCOMMON', 70, 'редкий', 'спутник встречается реже обычного', Rarity(), RARITY_BIGER, False),
        ('RARE', 71, 'очень редкий', 'спутник встречается очень редко', Rarity(), RARITY_BIGEST, False),
        ('SPECIAL', 72, 'особый', 'особый спутник, которого нельзя получить обычным способом', Rarity(), RARITY_LEGENDARY, False),

        ('TEMPORARY', 73, 'временный', 'спутник с небольшой вероятностью может покинуть героя при посещении города',
         Unsociable(0.1 * c.COMPANIONS_LEAVE_IN_PLACE), RARITY_LOWEST, True),
        ('UNSOCIABLE', 74, 'нелюдимый', 'спутник может покинуть героя при посещении города',
         Unsociable(c.COMPANIONS_LEAVE_IN_PLACE), RARITY_LOWEST_2, True)
    )
