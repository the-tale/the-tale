...
class RISK_LEVEL(rels_django.DjangoEnum):
    ...
    rung_bonus = rels.Column()

    description = rels.Column()

    records = (('VERY_HIGH', 0, 'очень высокий', 0.70, 1.30, 2.00, 0.0, 0.5,
                'Больше опыта, наград и влияния за задания, +200% к денежным наградам за задания. При поиске пути герой полностью игнорирует безопасность региона.'),
               ('HIGH', 1, 'высокий', 0.85, 1.15, 1.00, 0.5, 0.25,
                'Немного больше опыта, наград и влияния за задания, +100% к денежным наградам за задания. При поиске пути герой ценит транспорт региона больше безопасности'),
               ('NORMAL', 2, 'обычный', 1.00, 1.00, 0.00, 1.0, 0.0,
                'Никакого влияния на опыт, награды и влияние героя. При поиске пути герой одинаково учитывает и транспорт и безопасность региона.'),
               ('LOW', 3, 'низкий', 1.15, 0.85, -1.00, 2.0, -0.25,
                'Немного меньше опыта, наград и влияния за задания, -100% к денежным наградам за задания. При поиске пути герой ценит безопасность региона больше транспорта.'),
               ('VERY_LOW', 4, 'очень низкий', 1.30, 0.70, -2.00, 4.0, -0.5,
                'Меньше опыта, наград и влияния за задания, -200% к денежным наградам за задания. При поиске пути герой оценивает безопасность региона значительно сильнее транспорта.'))
