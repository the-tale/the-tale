...
_xls_attributes = {'filename': conf.settings.TERRAIN_PRIORITIES_FIXTURE,
                   'rows': [terrain.name for terrain in relations.TERRAIN.records]}


def _modify_row_names(d): return dict((getattr(relations.TERRAIN, key), value) for key, value in list(d.items()))
