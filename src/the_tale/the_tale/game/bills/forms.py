...
class BaseUserForm(forms.Form):
    ...
    caption = fields.CharField(label='Название закона', max_length=Bill.CAPTION_MAX_LENGTH, min_length=Bill.CAPTION_MIN_LENGTH)
    rationale = bbcode.BBField(label='', min_length=RATIONALE_MIN_LENGTH)

    chronicle_on_accepted = fields.TextField(label='Запись в летопись при принятии', widget=Textarea(attrs={'rows': 2}),
