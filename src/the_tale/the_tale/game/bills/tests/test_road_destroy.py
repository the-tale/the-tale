...
class RoadDestroyTests(helpers.BaseTestPrototypes):

    def setUp(self):
        super().setUp()

        self.assertNotEqual(roads_logic.road_between_places(self.place1, self.place2), None)

        self.bill_data = bills.road_destroy.RoadDestroy(place_1_id=self.place1.id,
    ...
    def test_user_form_validation__not_exists(self):
        self.assertFalse(roads_logic.road_between_places(self.place1, self.place3))
        form = self.bill.data.get_user_form_update(post={'caption': 'long caption',
                                                         'chronicle_on_accepted': 'chronicle-on-accepted',
                                                         'place_1': self.place1.id,
                                                         'place_2': self.place3.id})
    ...
    def test_apply(self):
        ...
        self.assertEqual(roads_logic.road_between_places(self.place1, self.place2), None)

    def test_has_meaning__not_exists(self):
        bill_data = bills.road_destroy.RoadDestroy(place_1_id=self.place1.id,
