...
class PERSONALITY_COSMETIC(PERSONALITY):
    records = ( personality('TRUTH_SEEKER', 0, 'правдолюб', 'ON_QUEST_HABITS', {QUEST_RESULTS.SUCCESSED: heroes_relations.HABIT_CHANGE_SOURCE.MASTER_QUEST_HONORABLE,
                ...
                'поручитель', 'поручительница', 'За выполнение задания, связанного с мастером, герой получает больше очков помощи в каждом городе, связанном с заданием.'),

                personality('NIHILIST', 7, 'нигилист', 'PLACES_HELP_AMOUNT', -0.5,
                'нигилист', 'нигилистка', 'За выполнение задания, связанного с мастером, герой получает меньше очков помощи в каждом городе, связанном с заданием.'),
