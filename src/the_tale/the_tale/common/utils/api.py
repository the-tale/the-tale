...
def handler(versions):
    ...
    def decorator(view):
        ...
        def view_wrapper(resource, **kwargs):
            ...
            if not api_version:
                return resource.json_error('api.no_method_version', 'Не указана версия метода')

            if not api_client:
                return resource.json_error('api.no_client_identificator', 'Не указана версия клиента')
            ...
            if api_version not in versions:
                return resource.json_error('api.wrong_method_version',
                                           'Неверная версия метода, ожидается одна из: %s' % ', '.join(versions))
