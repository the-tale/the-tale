...
class XSOLLA_RESULT(rels_django.DjangoEnum):
...
class COMMAND_TYPE(rels_django.DjangoEnum):
    records = (('CHECK', 'check', 'проверка существования пользователя'),
               ('PAY', 'pay', 'зачисление средств'),
               ('CANCEL', 'cancel', 'отмена платежа'))


class COMMON_RESULT(rels_django.DjangoEnum):
    xsolla_result = rels.Column(unique=False, primary=False)
...
class CANCEL_RESULT(rels_django.DjangoEnum):
    xsolla_result = rels.Column(unique=False, primary=False)
...
class CHECK_USER_RESULT(rels_django.DjangoEnum):
    xsolla_result = rels.Column(unique=False, primary=False)
...
class PAY_RESULT(rels_django.DjangoEnum):
    xsolla_result = rels.Column(unique=False, primary=False)
    invoice_state = rels.Column(unique=False, primary=False)
