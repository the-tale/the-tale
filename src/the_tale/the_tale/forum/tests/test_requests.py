...
class ForumResourceReadAllInSubcategoryTests(BaseTestRequests):
    ...
    def test_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:read-all-in-subcategory', self.subcat1.id)), 'common.login_required')
        self.assertEqual(prototypes.SubCategoryReadInfoPrototype._db_count(), 0)

    def test_success(self):
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:read-all-in-subcategory', self.subcat1.id)))
...
class ForumResourceReadAllTests(BaseTestRequests):
    ...
    def test_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:read-all')), 'common.login_required')
        self.assertEqual(prototypes.SubCategoryReadInfoPrototype._db_count(), 0)

    def test_success(self):
        self.assertTrue(prototypes.SubCategoryPrototype._db_count() > 0)

        self.check_ajax_ok(self.client.post(dext_urls.url('forum:read-all')))
...
class TestRequests(BaseTestRequests):
    ...
    def test_index(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:')), texts=texts)
        self.request_logout()
        self.check_html_ok(self.request_html(dext_urls.url('forum:')), texts=texts)
...
class TestSubcategoryRequests(BaseTestRequests):
    ...
    def test_subcategory__unlogined(self):
        ...
        self.request_logout()
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:show', self.subcat1.id)), texts=texts)
    ...
    def test_subcategory(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:show', self.subcat1.id)), texts=[('pgf-new-thread-marker', 0)])
    ...
    def test_second_page(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:show', self.subcat1.id, page=2)), texts=texts)
    ...
    def test_importants_threads_on_first_page(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:show', self.subcat1.id, page=1)), texts=[self.thread2.caption])

    @mock.patch('the_tale.forum.prototypes.SubCategoryPrototype.is_restricted_for', lambda proto, account: True)
    def test_restricted(self):
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:show', self.subcat1.id)), texts=['forum.subcategory_access_restricted'])

    def test_subcategory_not_found(self):
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:show', 666)), texts=[('forum.subcategory.not_found', 1)], status_code=404)
...
class TestNewThreadRequests(BaseTestRequests):
    ...
    def test_new_thread(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:new-thread', self.subcat1.id)), texts=texts)

    def test_new_thread_unlogined(self):
        self.request_logout()
        request_url = dext_urls.url('forum:subcategories:new-thread', self.subcat1.id)
        self.check_redirect(request_url, accounts_logic.login_page_url(request_url))

    def test_new_thread_fast(self):
        self.account.is_fast = True
        self.account.save()
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:new-thread', self.subcat1.id)), texts=['pgf-error-common.fast_account'])

    @mock.patch('the_tale.accounts.prototypes.AccountPrototype.is_ban_forum', True)
    def test_new_thread_banned(self):
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:new-thread', self.subcat1.id)), texts=['pgf-error-common.ban_forum'])

    @mock.patch('the_tale.forum.prototypes.SubCategoryPrototype.is_restricted_for', lambda proto, account: True)
    def test_restricted(self):
        self.check_html_ok(self.request_html(dext_urls.url('forum:subcategories:new-thread', self.subcat1.id)), texts=['forum.subcategory_access_restricted'])
...
class TestCreateThreadRequests(BaseTestRequests):
    ...
    def test_create_thread_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:create-thread', self.subcat1.id)),
    ...
    def test_create_thread_closed_subcategory(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:create-thread', self.subcat2.id)),
    ...
    def test_create_thread_form_errors__empty_caption(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:create-thread', self.subcat1.id), {'caption': '', 'text': 'thread4-text'}),
                              code='forum.create_thread.form_errors')

    def test_create_thread_form_errors__empty_body(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:create-thread', self.subcat1.id), {'caption': 'thread4-caption', 'text': ''}),
                              code='forum.create_thread.form_errors')

    def test_create_thread_success(self):
        response = self.client.post(dext_urls.url('forum:subcategories:create-thread', self.subcat1.id), {'caption': 'thread4-caption', 'text': 'thread4-text'})
    ...
    @mock.patch('the_tale.forum.prototypes.SubCategoryPrototype.is_restricted_for', lambda proto, account: True)
    def test_restricted(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:create-thread', self.subcat1.id)), 'forum.subcategory_access_restricted')

    @mock.patch('the_tale.forum.prototypes.ThreadPrototype.get_new_thread_delay', classmethod(lambda cls, account: 1))
    def test_delayed(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:create-thread', self.subcat1.id), {'caption': 'thread4-caption', 'text': 'thread4-text'}),
...
class TestIndexThreadsRequests(BaseTestRequests):
    ...
    def setUp(self):
        ...
        self.index_url = dext_urls.url('forum:threads:')
...
class TestShowThreadRequests(BaseTestRequests):
    ...
    def test_get_thread_unlogined(self):
        self.request_logout()
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread1.id)), texts=(('pgf-new-post-form', 0),
    ...
    def test_get_thread_not_found(self):
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', 666)), texts=(('forum.thread.not_found', 1),), status_code=404)
    ...
    def test_get_thread__posts_not_edited(self):
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread1.id)), texts=(('pgf-edit-time-message', 0),))

    def test_get_thread__posts_edited(self):
        post = self.thread1.get_last_post()
        post.save()
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread1.id)), texts=('pgf-edit-time-message'))

    def test_get_thread(self):
        self.assertEqual(prototypes.ThreadReadInfoPrototype._db_count(), 0)
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread1.id)), texts=('pgf-new-post-form',))
    ...
    def test_get_thread__complaint_button(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread1.id)), texts=(('pgf-complaint-button', 0), ))

        prototypes.PostPrototype.create(self.thread1, self.account_2, 'post2-text')
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread1.id)), texts=(('pgf-complaint-button', 1), ))

    def test_get_thread_wrong_page(self):
        response = self.request_html(dext_urls.url('forum:threads:show', self.thread1.id) + '?page=2')
        self.assertRedirects(response, dext_urls.url('forum:threads:show', self.thread1.id) + '?page=1', status_code=302, target_status_code=200)
    ...
    def test_get_thread_with_pagination(self):
        ...
        response = self.request_html(dext_urls.url('forum:threads:show', self.thread3.id) + '?page=2')
        self.assertRedirects(response, dext_urls.url('forum:threads:show', self.thread3.id) + '?page=1', status_code=302, target_status_code=200)

        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread3.id)), texts=texts)

        text = 'subcat3-post%d-text' % (conf.settings.POSTS_ON_PAGE)
        prototypes.PostPrototype.create(self.thread3, self.account, text)
        texts.append((text, 0))

        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread3.id) + '?page=1'), texts=texts)
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread3.id) + '?page=2'), texts=[text])
    ...
    @mock.patch('the_tale.forum.prototypes.SubCategoryPrototype.is_restricted_for', lambda proto, account: True)
    def test_restricted(self):
        self.check_html_ok(self.request_html(dext_urls.url('forum:threads:show', self.thread1.id)), texts=('forum.subcategory_access_restricted',))
...
class TestCreatePostRequests(BaseTestRequests):
    ...
    def test_create_post_unlogined(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:threads:create-post', self.thread3.id)),
                              code='common.login_required')
    ...
    def test_create_post_form_errors__empty_post(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:threads:create-post', self.thread3.id), {'text': ''}),
                              code='forum.create_post.form_errors')
    ...
    def test_create_post_success(self):
        ...
        response = self.client.post(dext_urls.url('forum:threads:create-post', self.thread3.id), {'text': 'thread3-test-post'})

        post = prototypes.PostPrototype._db_get_object(4)

        self.check_ajax_ok(response, data={'next_url': dext_urls.url('forum:threads:show', self.thread3.id) + ('?page=1#m%d' % post.id)})
    ...
    def test_create_post_thread_not_found(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:threads:create-post', 666), {'text': 'thread3-test-post'}),
                              'forum.thread.not_found')

    @mock.patch('the_tale.forum.prototypes.SubCategoryPrototype.is_restricted_for', lambda proto, account: True)
    def test_restricted(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:threads:create-post', self.thread3.id), {'text': 'thread3-test-post'}),
    ...
    def test_delayed(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:threads:create-post', self.thread3.id), {'text': 'post4-text'}),
...
class TestFeedRequests(BaseTestRequests):
    ...
    def test_feed_page(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:feed')), texts=texts, content_type='application/atom+xml')
...
class ThreadSubscribeTests(BaseTestRequests):
    ...
    def test_login_required(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:threads:subscribe', self.thread1.id)),
    ...
    def test_create_for_thread(self):
        ...
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:threads:subscribe', self.thread1.id)))
...
class SubcategorySubscribeTests(BaseTestRequests):
    ...
    def test_login_required(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:subscribe', self.subcat2.id)), 'common.login_required')

    def test_fast_account(self):
        self.account.is_fast = True
        self.account.save()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:subscribe', self.subcat2.id)), 'common.fast_account')

    def test_create_for_subcategory(self):
        self.assertEqual(models.Subscription.objects.all().count(), 0)
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:subcategories:subscribe', self.subcat2.id)))
        self.assertEqual(models.Subscription.objects.all().count(), 1)
    ...
    @mock.patch('the_tale.forum.prototypes.SubCategoryPrototype.is_restricted_for', lambda proto, account: True)
    def test_restricted(self):
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:subscribe', self.subcat2.id)),
                              'forum.subcategory_access_restricted')
...
class ThreadUnsubscribeTests(BaseTestRequests):
    ...
    def test_login_required(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:threads:unsubscribe', self.thread1.id)),
                              'common.login_required')
    ...
    def test_remove_for_thread(self):
        self.assertEqual(models.Subscription.objects.all().count(), 1)
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:threads:unsubscribe', self.thread1.id)))
        self.assertEqual(models.Subscription.objects.all().count(), 0)
...
class SubcategoryUnsubscribeTests(BaseTestRequests):
    ...
    def test_login_required(self):
        self.request_logout()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subcategories:unsubscribe', self.subcat2.id)),
                              'common.login_required')
    ...
    def test_remove_for_subcategory(self):
        self.assertEqual(models.Subscription.objects.all().count(), 1)
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:subcategories:unsubscribe', self.subcat2.id)))
        self.assertEqual(models.Subscription.objects.all().count(), 0)
...
class SubscriptionsTests(BaseTestRequests):
    ...
    def test_login_required(self):
        self.request_logout()
        request_url = dext_urls.url('forum:subscriptions:')
        self.check_redirect(request_url, accounts_logic.login_page_url(request_url))

    def test_fast_account(self):
        self.account.is_fast = True
        self.account.save()
        self.check_ajax_error(self.client.post(dext_urls.url('forum:subscriptions:')),
                              'common.fast_account')
    ...
    def test_empty(self):
        ...
        self.check_html_ok(self.request_html(dext_urls.url('forum:subscriptions:')), texts=texts)

    def test_subscriptions(self):
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:threads:subscribe', self.thread1.id)))
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:threads:subscribe', self.thread3.id)))
        self.check_ajax_ok(self.client.post(dext_urls.url('forum:subcategories:subscribe', self.subcat2.id)))
