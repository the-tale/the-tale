...
class BaseCollectionsResource(utils_resources.Resource):
    ...
    @old_views.validate_argument('collection', lambda value: storage.collections[int(value)], 'collections', 'Коллекция не найдена')
    @old_views.validate_argument('kit', lambda value: storage.kits[int(value)], 'collections', 'Набор не найден')
    @old_views.validate_argument('item', lambda value: storage.items[int(value)], 'collections', 'Предмет не найден')
    ...
    def collection_url(self, collection):
        if self.master_account:
            return utils_urls.url('collections:collections:show', collection.id, account=self.master_account.id)
        else:
            return utils_urls.url('collections:collections:show', collection.id)

    def index_url(self):
        if self.master_account:
            return utils_urls.url('collections:collections:', account=self.master_account.id)
        else:
            return utils_urls.url('collections:collections:')


@old_views.validator(code='collections.collections.no_edit_rights', message='нет прав для редактирования коллекции')
def validate_can_edit_collection(resource, *args, **kwargs):
    return resource.can_edit_collection


@old_views.validator(code='collections.collections.no_moderate_rights', message='нет прав для модерации коллекции')
def validate_can_moderate_collection(resource, *args, **kwargs):
    return resource.can_moderate_collection


@old_views.validator(code='collections.collections.not_approved', message='коллекция не найдена', status_code=404)
...
class CollectionsResource(BaseCollectionsResource):
    ...
    @old_views.validate_argument('account', accounts_prototypes.AccountPrototype.get_by_id, 'collections', 'Игрок не найден')
    @old_views.handler('')
    def index(self, account=None):

        if account is None and self.account.is_authenticated:
            return self.redirect(utils_urls.url('collections:collections:', account=self.account.id))
    ...
    def create(self):
        ...
        return self.json_ok(data={'next_url': utils_urls.url('collections:collections:show', collection.id)})

    @validate_collection_approved()
    @old_views.validate_argument('account', accounts_prototypes.AccountPrototype.get_by_id, 'collections', 'Игрок не найден')
    @old_views.handler('#collection', name='show')
    def show(self, account=None):

        if account is None and self.account.is_authenticated:
            return self.redirect(utils_urls.url('collections:collections:show', self.collection.id, account=self.account.id))
    ...
    @utils_decorators.login_required
    @validate_can_edit_collection()
    @old_views.handler('#collection', 'edit')
    def edit(self):
    ...
    @utils_decorators.login_required
    @validate_can_edit_collection()
    @old_views.handler('#collection', 'update')
    ...
    @old_views.handler('#collection', 'approve')
    ...
    @utils_decorators.login_required
    @validate_can_moderate_collection()
    @old_views.handler('#collection', 'disapprove')
...
@old_views.validator(code='collections.kits.no_edit_rights', message='нет прав для редактирования набора')
def validate_can_edit_kit(resource, *args, **kwargs):
    return resource.can_edit_kit or resource.can_moderate_kit


@old_views.validator(code='collections.kits.no_moderate_rights', message='нет прав для модерации набора')
def validate_can_moderate_kit(resource, *args, **kwargs):
    return resource.can_moderate_kit


@old_views.validator(code='collections.kits.not_approved', message='набор не найден', status_code=404)
...
class KitsResource(BaseCollectionsResource):
    ...
    def create(self):
        ...
        return self.json_ok(data={'next_url': utils_urls.url('collections:collections:show', kit.collection_id)})

    @utils_decorators.login_required
    @validate_can_edit_kit()
    @old_views.handler('#kit', 'edit')
    def edit(self):
    ...
    def update(self, method='post'):
        ...
        return self.json_ok(data={'next_url': utils_urls.url('collections:collections:show', self.kit.collection_id)})

    @utils_decorators.login_required
    @validate_can_moderate_kit()
    @old_views.handler('#kit', 'approve')
    def approve(self, method='post'):
    ...
    @utils_decorators.login_required
    @validate_can_moderate_kit()
    @old_views.handler('#kit', 'disapprove')
    def disapprove(self, method='post'):
...
@old_views.validator(code='collections.items.no_edit_rights', message='нет прав для редактирования предмета')
def validate_can_edit_item(resource, *args, **kwargs):
    return resource.can_edit_item or resource.can_moderate_item


@old_views.validator(code='collections.items.no_moderate_rights', message='нет прав для модерации предмета')
...
class ItemsResource(BaseCollectionsResource):
    ...
    def create(self):
        ...
        return self.json_ok(data={'next_url': utils_urls.url('collections:collections:show', item.kit.collection_id)})

    @utils_decorators.login_required
    @validate_can_edit_item()
    @old_views.handler('#item', 'edit')
    def edit(self):
    ...
    def update(self, method='post'):
        ...
        return self.json_ok(data={'next_url': utils_urls.url('collections:collections:show', self.item.kit.collection_id)})

    @utils_decorators.login_required
    @validate_can_moderate_item()
    @old_views.handler('#item', 'approve')
    def approve(self, method='post'):
    ...
    @utils_decorators.login_required
    @validate_can_moderate_item()
    @old_views.handler('#item', 'disapprove')
    def disapprove(self, method='post'):
