...
class GuideResource(utils_resources.Resource):
    ...
    @dext_old_views.handler('cities', method='get')
    def cities(self):
    ...
    @dext_old_views.handler('politics', method='get')
    def politics(self):
    ...
    @dext_old_views.handler('clans', method='get')
    def clans(self):
        return self.template('guide/clans.html',
                             {'section': 'clans',
                              'clans_settings': clans_conf.settings})

    @dext_old_views.handler('map', method='get')
    def map(self):
        return self.template('guide/map.html', {'section': 'map'})

    @dext_old_views.handler('pvp', method='get')
    def pvp(self):
        return self.template('guide/pvp.html', {'section': 'pvp',
                                                'pvp_settings': pvp_conf.settings,
                                                'pvp_abilities': pvp_abilities})

    @dext_old_views.handler('api', method='get')
    def api(self):
        return self.template('guide/api.html', {'section': 'api',
                                                'types': API_TYPES})

    @dext_old_views.validate_argument('ability_type', lambda x: heroes_abilities_relations.ABILITY_TYPE(int(x)), 'guide.hero_abilities', 'Неверный формат типа способности')
    @dext_old_views.validate_argument('activation_type', lambda x: heroes_abilities_relations.ABILITY_ACTIVATION_TYPE(int(x)), 'guide.hero_abilities', 'Неверный формат типа активации')
    @dext_old_views.validate_argument('availability', lambda x: heroes_abilities_relations.ABILITY_AVAILABILITY(int(x)), 'guide.hero_abilities', 'Неверный формат типа доступности')
    @dext_old_views.handler('hero-abilities', method='get')
    def hero_abilities(self, ability_type=None, activation_type=None, availability=heroes_abilities_relations.ABILITY_AVAILABILITY.FOR_ALL):
        ...
        url_builder = dext_urls.UrlBuilder(django_reverse('guide:hero-abilities'), arguments={'ability_type': ability_type.value if ability_type is not None else None,
    ...
    @dext_old_views.handler('hero-preferences', method='get')
    ...
    @dext_old_views.validate_argument('habit', lambda x: game_relations.HABIT_TYPE(int(x)), 'guide.hero_habit_info', 'Неверный тип черты')
    @dext_old_views.handler('hero-habit-info', method='get')
    def habit_info(self, habit=game_relations.HABIT_TYPE.HONOR):
        return self.template('guide/hero-habit-info.html', {'habit': habit,
                                                            'HABIT_TYPE': game_relations.HABIT_TYPE})

    @dext_old_views.handler('press-kit', name='press-kit')
    def press_kit(self):
        return self.template('guide/press_kit.html',
                             {'section': 'press-kit',
                              'mob': random.choice(mobs_storage.mobs.get_all_mobs_for_level(level=666))})

    @dext_old_views.handler('world')
    def world(self):
        return self.template('guide/world.html', {'section': 'world'})

    @dext_old_views.handler('how-to-help')
