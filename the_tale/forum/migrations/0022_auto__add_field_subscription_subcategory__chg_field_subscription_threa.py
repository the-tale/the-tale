...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Subscription.subcategory'
        db.add_column(u'forum_subscription', 'subcategory',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['forum.SubCategory'], null=True),
                      keep_default=False)


        # Changing field 'Subscription.thread'
        db.alter_column(u'forum_subscription', 'thread_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['forum.Thread'], null=True))
        # Adding unique constraint on 'Subscription', fields ['account', 'subcategory']
        db.create_unique(u'forum_subscription', ['account_id', 'subcategory_id'])

    def backwards(self, orm):
        # Removing unique constraint on 'Subscription', fields ['account', 'subcategory']
        db.delete_unique(u'forum_subscription', ['account_id', 'subcategory_id'])

        # Deleting field 'Subscription.subcategory'
        db.delete_column(u'forum_subscription', 'subcategory_id')


        # User chose to not deal with backwards NULL issues for 'Subscription.thread'
        raise RuntimeError("Cannot reverse this migration. 'Subscription.thread' and its values cannot be restored.")
    models = {
        u'accounts.account': {
