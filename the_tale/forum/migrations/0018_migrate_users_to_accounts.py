...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for subcategory in orm['forum.SubCategory'].objects.all():
            if subcategory.user_last_poster:
                subcategory.last_poster = orm['accounts.Account'].objects.get(user=subcategory.user_last_poster)

            subcategory.save()

        for thread in orm['forum.Thread'].objects.all():
            if thread.user_last_poster:
                thread.last_poster = orm['accounts.Account'].objects.get(user=thread.user_last_poster)

            thread.author = orm['accounts.Account'].objects.get(user=thread.user_author)
            thread.save()

        for post in orm['forum.Post'].objects.all():
            post.author = orm['accounts.Account'].objects.get(user=post.user_author)

            if post.user_remove_initiator:
                post.remove_initiator = orm['accounts.Account'].objects.get(user=post.user_remove_initiator)
