...
class TestRequests(TestCase):
    ...
    def test_subcategory(self):
        ...
        self.check_html_ok(self.client.get(reverse('forum:subcategory', args=['cat1-slug', 'subcat1-slug'])), texts=texts)
        self.logout()
        self.check_html_ok(self.client.get(reverse('forum:subcategory', args=['cat1-slug', 'subcat1-slug'])), texts=texts)

    def test_new_thread(self):
        texts=['cat1-caption', 'subcat1-caption']
        self.check_html_ok(self.client.get(reverse('forum:new_thread', args=['cat1-slug', 'subcat1-slug'])), texts=texts)

    def test_new_thread_unlogined(self):
        self.logout()
        self.check_html_ok(self.client.get(reverse('forum:new_thread', args=['cat1-slug', 'subcat1-slug'])), texts=['pgf-error-forum.new_thread.unlogined'])

    def test_new_thread_fast(self):
        account = self.user.get_profile()
        account.is_fast = True
        account.save()
        self.check_html_ok(self.client.get(reverse('forum:new_thread', args=['cat1-slug', 'subcat1-slug'])), texts=['pgf-error-forum.new_thread.fast_account'])

    def test_create_thread_unlogined(self):
        self.logout()
        self.check_ajax_error(self.client.post(reverse('forum:create_thread', args=['cat1-slug', 'subcat1-slug'])),
    ...
    def test_create_thread_closed_subcategory(self):
        self.check_ajax_error(self.client.post(reverse('forum:create_thread', args=['cat1-slug', 'subcat2-slug'])),
    ...
    def test_create_thread_success(self):
        response = self.client.post(reverse('forum:create_thread', args=['cat1-slug', 'subcat1-slug']), {'caption': 'thread4-caption', 'text': 'thread4-text'})
    ...
    def test_get_thread_unlogined(self):
        ...
        self.logout()
        self.check_html_ok(self.client.get(reverse('forum:show_thread', args=['cat1-slug', 'subcat1-slug', self.thread1.id])), excluded_texts=excluded_texts)
    ...
    def test_get_thread_wrong_page(self):
        response = self.client.get(reverse('forum:show_thread', args=['cat1-slug', 'subcat1-slug', self.thread1.id])+'?page=2')
        self.assertRedirects(response, reverse('forum:show_thread', args=['cat1-slug', 'subcat1-slug', self.thread1.id])+'?page=1', status_code=302, target_status_code=200)
    ...
    def test_get_thread_with_pagination(self):
        ...
        response = self.client.get(reverse('forum:show_thread', args=['cat3-slug', 'subcat3-slug', self.thread3.id])+'?page=2')
        self.assertRedirects(response, reverse('forum:show_thread', args=['cat3-slug', 'subcat3-slug', self.thread3.id])+'?page=1', status_code=302, target_status_code=200)

        self.check_html_ok(self.client.get(reverse('forum:show_thread', args=['cat3-slug', 'subcat3-slug', self.thread3.id])), texts=texts)
        ...
        self.check_html_ok(self.client.get(reverse('forum:show_thread', args=['cat3-slug', 'subcat3-slug', self.thread3.id])+'?page=1', texts=texts))
        response = self.client.get(reverse('forum:show_thread', args=['cat3-slug', 'subcat3-slug', self.thread3.id])+'?page=2', texts=[text])
    ...
    def test_create_post_unlogined(self):
        self.logout()
        self.check_ajax_error(self.client.post(reverse('forum:create_post', args=['cat1-slug', 'subcat1-slug', self.thread3.id])),
    ...
    def test_create_post_success(self):
        self.check_ajax_ok(self.client.post(reverse('forum:create_post', args=['cat1-slug', 'subcat1-slug', self.thread3.id]), {'text': 'thread3-test-post'}))
        self.assertEqual(Post.objects.all().count(), 5)
