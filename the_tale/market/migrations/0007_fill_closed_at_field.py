...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for lot in orm['market.Lot'].objects.filter(state__in=(2, 3, 4)):
            lot.closed_at = lot.created_at + datetime.timedelta(days=3.5)
            lot.save()

        for lot in orm['market.Lot'].objects.exclude(state__in=(2, 3, 4)):
            lot.closed_at = lot.created_at + datetime.timedelta(days=7)
            lot.save()
