...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for quest in orm['quests.Quest'].objects.all():
            data = json.loads(quest.data)
            data['rewards'] = {'quest_%d'%i: {'experience': random.randint(50, 100),
                                              'power': random.randint(100, 200)}
                               for i in xrange(10)}
            quest.data = json.dumps(data)
            quest.save()
    ...
    models = {
        ...
        u'quests.questchoice': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'quest': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'choices'", 'to': u"orm['quests.Quest']"})
        },
