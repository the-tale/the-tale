...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for record in orm['bills.Bill'].objects.filter(created_at__gt=datetime.datetime(year=2013, month=1, day=11)):
            data = json.loads(record.technical_data)

            if 'old_name_forms' in data and data['old_name_forms']['forms'][0] == u'название неизвестно':
                data['old_name_forms'] = json.loads(orm['places.Place'].objects.get(id=data['place_id']).name_forms)
            if 'old_place_name_forms' in data and data['old_place_name_forms']['forms'][0] == u'название неизвестно':
                data['old_place_name_forms'] = json.loads(orm['places.Place'].objects.get(id=data['place_id']).name_forms)

            record.technical_data = json.dumps(data)
    ...
    models = {
        ...
        u'bills.bill': {
            ...
            'state': ('rels.django.RelationIntegerField', [], {'default': '1', 'db_index': 'True'}),
