...
class Migration(DataMigration):
    ...
    depends_on = (
        ("persons", "0009_auto__add_field_person_created_at__add_field_person_created_at_turn"),
    )
    ...
    def forwards(self, orm):
        ...
        for model in orm['bills.Bill'].objects.all():
            if model.type != 1: # filter non person_remove_bills
                continue
            data = json.loads(model.technical_data)

            person = orm['persons.Person'].objects.get(id=data['person_id'])
            data['person_name'] = person.name
            data['person_race'] = person.race
            data['person_type'] = person.type
            data['person_gender'] = person.gender
