...
class Migration(DataMigration):
    ...
    def create_name_data(self, name):
        return { "forms": [name]*12,
                 "type": 1,
                 "normalized": name,
                 "properties": ["мр"]}
    ...
    def forwards(self, orm):
        ...
        for record in orm['bills.Bill'].objects.all():
            data = json.loads(record.technical_data)

            if 'old_name' in data:
                data['old_name_forms'] = self.create_name_data(data['old_name'])
            if 'old_place_name' in data:
                data['old_place_name_forms'] = self.create_name_data(data['old_place_name'])
