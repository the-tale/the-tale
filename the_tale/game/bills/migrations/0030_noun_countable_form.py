...
from utg.migrations import m_0002_countable_noun_form


class Migration(DataMigration):

    def migrate_data(self, data, name, aninality=False):
        if name in data:
            data[name] = m_0002_countable_noun_form.migrate(data[name])


    def forwards(self, orm):
        for model in orm['bills.Bill'].objects.all():
            data = json.loads(model.technical_data)

            self.migrate_data(data, 'old_place_name_forms')
            self.migrate_data(data, 'building_name_forms')
            self.migrate_data(data, 'old_building_name_forms')
            self.migrate_data(data, 'new_building_name_forms')
            self.migrate_data(data, 'old_name_forms')
            self.migrate_data(data, 'name_forms')
            self.migrate_data(data, 'old_place_1_name_forms')
            self.migrate_data(data, 'old_place_2_name_forms')

            model.technical_data = json.dumps(data)
            model.save()
