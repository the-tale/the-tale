...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for model in orm['places.Place'].objects.all():
            textgen_data = json.loads(model.name_forms)
            data = json.loads(model.data)

            data['name'] = convert_textgen_noun_json_to_utg(textgen_data, aninality=False)

            model.data = json.dumps(data)
            model.save()

        for model in orm['places.Building'].objects.all():
            textgen_data = json.loads(model.name_forms)
            data = json.loads(model.data)
