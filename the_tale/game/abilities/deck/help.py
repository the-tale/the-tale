...
class Help(AbilityPrototype):
    ...
    def use(self, current_time, bundle, angel, hero, form):
        ...
        if choice == HELP_CHOICES.HEAL:
            ...
            hero.add_message('angel_ability_healhero', current_time, hero=hero, health=heal_amount)
            return True

        if choice == HELP_CHOICES.START_QUEST:
            action.init_quest()
            hero.add_message('angel_ability_stimulate', current_time, hero=hero)
            return True
        ...
        if choice == HELP_CHOICES.TELEPORT:
            ...
            hero.add_message('angel_ability_shortteleport', current_time, hero=hero)
            return True

        if choice == HELP_CHOICES.LIGHTING:
            action.bit_mob(random.uniform(*c.ANGEL_HELP_LIGHTING_FRACTION))
            hero.add_message('angel_ability_lightning', current_time, hero=hero, mob=action.mob)
            return True

        if choice == HELP_CHOICES.RESURRECT:
            action.fast_resurrect()
            hero.add_message('angel_ability_resurrect', current_time, hero=hero)
            return True
