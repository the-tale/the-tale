...
from the_tale.common.utils.constant_code import level_rebalance

def rebalance_item(item_data):
    old_level = item_data['level']
    new_level = level_rebalance.old_to_new(old_level, 0)[0]

    item_data['level'] = new_level

    power_1, power_2 = item_data['power']
    item_data['power'] = [int(float(power_1) / old_level * new_level + 1) if power_1 else 0,
                          int(float(power_2) / old_level * new_level + 1) if power_2 else 0]


class Migration(DataMigration):

    def forwards(self, orm):
        for hero in orm['heroes.Hero'].objects.all().iterator():
            hero.level, hero.experience = level_rebalance.old_to_new(hero.level, hero.experience)

            bag = json.loads(hero.bag)
            for item_data in bag.get('bag', {}).itervalues():
                rebalance_item(item_data)
            hero.bag = json.dumps(bag)

            equipment = json.loads(hero.equipment)
            for item_data in equipment.itervalues():
                rebalance_item(item_data)
            hero.equipment = json.dumps(equipment)
