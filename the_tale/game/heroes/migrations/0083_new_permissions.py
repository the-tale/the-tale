...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for hero in orm['heroes.Hero'].objects.all():
            hero.preferences = json.dumps({'mob': {'value': hero.pref_mob_id, 'changed_at': time.mktime(hero.pref_mob_changed_at.timetuple())},
                                           'place': {'value': hero.pref_place_id, 'changed_at': time.mktime(hero.pref_place_changed_at.timetuple())},
                                           'friend': {'value': hero.pref_friend_id, 'changed_at': time.mktime(hero.pref_friend_changed_at.timetuple())},
                                           'enemy': {'value': hero.pref_enemy_id, 'changed_at': time.mktime(hero.pref_enemy_changed_at.timetuple())},
                                           'energy_regeneration_type': {'value': int(hero.pref_energy_regeneration_type) if hero.pref_energy_regeneration_type is not None else None,
                                                                        'changed_at': time.mktime(hero.pref_energy_regeneration_type_changed_at.timetuple())},
                                           'equipment_slot': {'value': int(hero.pref_equipment_slot) if hero.pref_equipment_slot is not None else None,
                                                              'changed_at': time.mktime(hero.pref_equipment_slot_changed_at.timetuple())} })
            hero.save()
