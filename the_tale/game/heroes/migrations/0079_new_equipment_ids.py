...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for hero_model in orm['heroes.Hero'].objects.all():
            equipment = json.loads(hero_model.equipment)
            equipment = {NEW_IDS[old_id]: value for old_id, value in equipment.items()}
            hero_model.equipment = json.dumps(equipment)
            hero_model.pref_equipment_slot = '%d' % NEW_IDS[hero_model.pref_equipment_slot] if hero_model.pref_equipment_slot else None
            hero_model.save()
