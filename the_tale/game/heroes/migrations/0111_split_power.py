...
class Migration(DataMigration):
    depends_on = ( ("artifacts", "0014_auto__del_field_artifactrecord_rarity__add_field_artifactrecord_power_"), )

    def forwards(self, orm):
        for hero in orm['heroes.Hero'].objects.all():
            bag = json.loads(hero.bag)
            for artifact in bag['bag'].values():
                artifact['power'] = [int(artifact['power']/2)+1,
                                     int(artifact['power']/2)+1]
            hero.bag = json.dumps(bag)

            equipment = json.loads(hero.equipment)
            for artifact in equipment.values():
                artifact['power'] = [int(artifact['power']/2)+1,
                                     int(artifact['power']/2)+1]
