...
FIXTURE = os.path.join(os.path.dirname(__file__), '../fixtures/mobs_damage_type.csv')

DAMAGE_TYPE = {u'магический': 0,
               u'смешаный': 1,
               u'физический': 2}
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for line in data.split('\n'):
            cells = line.split(',')
            mob_id = int(cells[0])
            mob_level = int(cells[2])
            mob_damage_type = DAMAGE_TYPE[cells[3]]
            orm['mobs.MobRecord'].objects.filter(id=mob_id).update(level=mob_level, archetype=mob_damage_type)
