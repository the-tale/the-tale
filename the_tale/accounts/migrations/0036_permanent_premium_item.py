...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for account in orm['accounts.Account'].objects.all():
            if account.premium_end_at < datetime.datetime.now() + datetime.timedelta(days=100*350):
                continue

            purchases = json.loads(account.permanent_purchases)
            purchases.append(12)

            account.permanent_purchases = json.dumps(purchases)
            account.save()
