...
class Migration(DataMigration):
    ...
    def add_achievement(self, orm, account_id, achievement):
        achievements = orm['achievements.AccountAchievements'].objects.get(account_id=account_id)
        data = json.loads(achievements.achievements)
        data['achievements'].append([achievement.id, time.time()])
        achievements.achievements = json.dumps(data)
        achievements.points += achievement.points
        achievements.save()


    def forwards(self, orm):
        Achievement = orm['achievements.Achievement']
        queen = Achievement.objects.create(group=11,
                                           type=18,
                                           caption=u'Королева Хранителей 2015',
                                           description=u'Стать королевой Хранителей в 2015 году',
                                           order=0,
                                           barrier=0,
                                           approved=True,
                                           points=50)
        princess = Achievement.objects.create(group=11,
                                              type=18,
                                              caption=u'Принцесса Хранителей 2015',
                                              description=u'Стать принцессой Хранителей в 2015 году',
                                              order=1,
                                              barrier=0,
                                              approved=True,
                                              points=25)

        self.add_achievement(orm, 18362, queen)
        self.add_achievement(orm, 4133, princess)
        self.add_achievement(orm, 14065, princess)
