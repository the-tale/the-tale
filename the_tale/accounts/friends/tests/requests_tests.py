...
class FriendshipRequestsTests(testcase.TestCase):
    ...
    def test_index__no_friends(self):
        self.check_html_ok(self.client.get(url('accounts:friends:')), texts=['alert-info pgf-no-friends-message'])
    ...
    def test_index(self):
        ...
        self.check_html_ok(self.client.get(url('accounts:friends:')), texts=[('alert-info pgf-no-friends-message', 0),
                                                                             (self.account_2.nick, 1),
                                                                             (self.account_3.nick, 1)])

    def test_candidates__no_candidates(self):
        self.check_html_ok(self.client.get(url('accounts:friends:candidates')), texts=['alert-info pgf-no-candidates-message'])
    ...
    def test_candidates(self):
        ...
        self.check_html_ok(self.client.get(url('accounts:friends:candidates')), texts=[('alert-info pgf-no-candidates-message', 0),
                                                                                       (self.account_2.nick, 1),
                                                                                       (self.account_3.nick, 1)])
