...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for template in orm['linguistics.Template'].objects.filter(key__in=[290000, 290001, 290002, 290003, 290004]):
            template.raw_template = template.raw_template.replace('hero', 'companion_owner').replace('mob', 'attacker')
            template.data = template.data.replace('hero', 'companion_owner').replace('mob', 'attacker')
            template.save()
