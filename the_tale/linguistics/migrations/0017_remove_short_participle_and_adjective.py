...
from utg.migrations import m_0001_include_short_forms_in_full

class Migration(DataMigration):

    def forwards(self, orm):
        Word = orm['linguistics.Word']

        Word.objects.filter(type__in=[8, 9]).delete()

        for word in Word.objects.all():
            word_data = json.loads(word.forms)

            word_data = m_0001_include_short_forms_in_full.migrate(word_data)

            word.forms = json.dumps(word_data)
            word.save()
