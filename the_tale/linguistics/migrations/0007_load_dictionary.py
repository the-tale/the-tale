...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        with open(os.path.join(os.path.join(project_settings.PROJECT_DIR, 'linguistics', 'fixtures', 'dictionary.json'))) as f:
            words = json.load(f)

        WordModel = orm['linguistics.Word']

        for word_data in words:
            utg_word = Word.deserialize(word_data)
            WordModel.objects.create(type=utg_word.type.value,
                                     state=1,
                                     normal_form=utg_word.normal_form(),
                                     forms=json.dumps(utg_word.serialize()))


    def backwards(self, orm):
        orm['linguistics.Word'].objects.all().delete()
