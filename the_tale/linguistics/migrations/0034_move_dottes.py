...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for template in orm['linguistics.Template'].objects.all():
             template.raw_template = template.raw_template.replace(u'...', u'…').replace(u'.»', u'».').replace('.\u00bb', '\u00bb.')
             template.data = template.data.replace(u'...', u'…').replace(u'.»', u'».').replace('.\u00bb', '\u00bb.')
             template.save()
