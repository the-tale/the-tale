...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_column(u'linguistics_template', 'template')

        # Adding field 'Template.author'
        db.add_column(u'linguistics_template', 'author',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['accounts.Account'], null=True, on_delete=models.SET_NULL),
                      keep_default=False)

        # Adding field 'Template.parent'
        db.add_column(u'linguistics_template', 'parent',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['linguistics.Template'], null=True, on_delete=models.SET_NULL),
                      keep_default=False)

        # Adding field 'Template.raw_template'
        db.add_column(u'linguistics_template', 'raw_template',
                      self.gf('django.db.models.fields.TextField')(default=u''),
                      keep_default=False)

        # Adding unique constraint on 'Template', fields ['raw_template', 'key', 'state']
        db.create_unique(u'linguistics_template', ['raw_template', 'key', 'state'])


    def backwards(self, orm):
        # Removing unique constraint on 'Template', fields ['raw_template', 'key', 'state']
        db.delete_unique(u'linguistics_template', ['raw_template', 'key', 'state'])


        # User chose to not deal with backwards NULL issues for 'Template.template'
        raise RuntimeError("Cannot reverse this migration. 'Template.template' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'Template.template'
        db.add_column(u'linguistics_template', 'template',
                      self.gf('django.db.models.fields.TextField')(),
                      keep_default=False)

        # Deleting field 'Template.author'
        db.delete_column(u'linguistics_template', 'author_id')

        # Deleting field 'Template.parent'
        db.delete_column(u'linguistics_template', 'parent_id')

        # Deleting field 'Template.raw_template'
        db.delete_column(u'linguistics_template', 'raw_template')
    ...
    models = {
        ...
        u'linguistics.template': {
            'Meta': {'unique_together': "(('raw_template', 'key', 'state'),)", 'object_name': 'Template'},
